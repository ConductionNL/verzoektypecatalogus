...
class HuwelijksplannerFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $stage2 = new Property();
        $stage2->addPrevious($stage1);
        $stage2->setTitle('indienen-babs-andere-gemeente');
        $stage2->setIcon('fas fa-user-friends');
        $stage2->setSlug('indienen-babs-andere-gemeente');
        $stage2->setDescription('Wie zijn de partners?');
        $stage2->setRequestType($request);
        $manager->persist($stage2);
        ...
        $stage2 = new Property();
        $stage2->addPrevious($stage1);
        $stage2->setTitle('indienen-babs-voor-een-dag');
        $stage2->setIcon('fas fa-user-friends');
        ...
        $stage2 = new Property();
        $stage2->addPrevious($stage1);
        $stage2->setTitle('afwijkende-trouw-locatie');
        $stage2->setIcon('fas fa-user-friends');
        $stage2->setSlug('afwijkende-trouw-locatie');
        $stage2->setDescription('Wie zijn de partners?');
        $stage2->setRequestType($request);
        $manager->persist($stage2);
        ...
        */
        ...
        $stage2->setMaxItems(2);
        $stage2->setRequired(true);
        $stage2->setDescription('Wie zijn de partners binnen dit huwelijk / partnerschap?');
        ...
        $id = Uuid::fromString('c253faeb-9a17-4db5-b1dc-f9f7b87847df');
        $stage4 = new Property();
        ...
        $manager->persist($stage4);
        $manager->flush();
        $stage4 = $manager->getRepository('App:Property')->findOneBy(['id' => $id]);

        $id = Uuid::fromString('5908ddf4-a221-405f-a981-02b76aeeae58');
        $stage5 = new Property();
        $stage5->setTitle('Locatie');
        $stage5->setIcon('fas fa-building');
        $stage5->setSlug('locatie');
        $stage5->setType('string');
        $stage5->setFormat('url');
        $stage5->setIri('pdc/offer');
        $stage5->setQuery(['audience'=>'public', 'products.groups.id'=>'170788e7-b238-4c28-8efc-97bdada02c2e']);
        $stage5->setRequired(true);
        $stage5->setDescription('Waar wilt u de voltrekking laten plaatsvinden?');
        $stage5->setRequestType($trouwenNL);
        $manager->persist($stage5);
        $stage5->setId($id);
        $manager->persist($stage5);
        $manager->flush();
        $stage5 = $manager->getRepository('App:Property')->findOneBy(['id' => $id]);

        $id = Uuid::fromString('bf0def7c-0fcf-4418-b960-fa3a8e66e6c1');
        $stage6 = new Property();
        ...
        $stage6->setQuery(['audience'=>'public', 'products.groups.id'=>'7f4ff7ae-ed1b-45c9-9a73-3ed06a36b9cc']);
        $stage6->setRequired(true);
        $stage6->setDescription('Door wie wilt u de plechtigheid laten voltrekken?');
        $stage6->setRequestType($trouwenNL);
        $manager->persist($stage6);
        $stage6->setId($id);
        $manager->persist($stage6);
        $manager->flush();
        $stage6 = $manager->getRepository('App:Property')->findOneBy(['id' => $id]);

        $id = Uuid::fromString('02d72f6c-cacb-4f41-a54f-f735c6dab1f6');
        $stage7 = new Property();
        ...
        $manager->persist($stage7);
        $manager->flush();
        $stage7 = $manager->getRepository('App:Property')->findOneBy(['id' => $id]);

        $id = Uuid::fromString('d29869c0-39ce-4691-a059-249e5ae0da9f');
        $stage8 = new Property();
        ...
        $stage8->setQuery(['audience'=>'public', 'products.groups.id'=>'f8298a12-91eb-46d0-b8a9-e7095f81be6f']);
        $stage8->setMinItems(1);
        $stage8->setRequired(true);
        $stage8->setDescription('Zijn er nog extra producten of diensten waar u gebruik van wilt maken?');
        $stage8->setRequestType($trouwenNL);
        $manager->persist($stage8);
        $stage8->setId($id);
        $manager->persist($stage8);
        $manager->flush();
        $stage8 = $manager->getRepository('App:Property')->findOneBy(['id' => $id]);

        $id = Uuid::fromString('ebbeed6e-2023-4b71-a6c7-31fcaa4fe3a6');
        $property = new Property();
        ...
        $id = Uuid::fromString('fcdbd0db-4cf2-48e1-aef4-16291ebb4b35');
        $property = new Property();
        ...
        $id = Uuid::fromString('025f4395-b90f-434f-91a4-a19bcf9de83d');
        $property = new Property();
        ...
        $id = Uuid::fromString('05db9c53-cf96-47d9-926f-0dac80cd3d61');
        $property = new Property();
